...
## 4.5 引擎智能层清单（继承 FinChip Generator 并通用化）

五层词法匹配（关键词/词干+同义词/字符n-gram余弦/DF权重/CJK单字层）、SymSpell 单键容错（clening→cleaning，STOP 词保护不误伤）、词汇表编译期反向织入、置信度三档（直答/didYouMean/兜底+AEO软着陆）、防复读惩罚、寒暄词法库、3-chip 组合律（支线传承+主线阶段先验+恒驻锚点）、兴趣簇偏置（通用化 persona：近3步访问轨迹投票）、深度≥4 面包屑回主干（↩ chip）、同线连接语。全部编译期冻结，零运行时学习。

**真双语输入**：宽松文本里同一条目 `问：`+`Q:` 成对、`答：`+`A:` 成对即为真双语节点；只写一种语言自动镜像。示例 `examples/input_restaurant.txt` 为 16 条全双语。
